...
function PhantOutput(config) {
  ...
  router.addRoute('/output/:publicKey/sample/:sample.:ext', responder.route.bind(responder, 'view'));
  router.addRoute('/output/:publicKey/sample/:sample', responder.route.bind(responder, 'view'));
  ...
  router.addRoute('/output/:publicKey/:field/sample/:sample.:ext', responder.route.bind(responder, 'view'));
  router.addRoute('/output/:publicKey/:field/sample/:sample', responder.route.bind(responder, 'view'));
...
app.view = function(req, res) {
  ...
  var self = this,
    ...
    pub = req.publicKey,
    page = req.query.page,
    limitStream, fieldStream, sampleStream, id;
  ...
  this.validator.exists(id, function(err, stream) {
    ...
    readStream.once('open', function() {
      ...
      if (req.sample) {
        sampleStream = SampleStream(req.sample);
        readStream = readStream.pipe(sampleStream);
      }
...
app.route = function(action, req, res, match) {
  ...
  req.sample = match.params.sample;
