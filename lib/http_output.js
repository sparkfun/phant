...
app.view = function(req, res) {
  ...
  var self = this,
    respond = this.pipe.bind(this, req, res),
...
app.stats = function(req, res) {
  ...
  this.validator.exists(id, function(err, stream) {
    ...
    self.storage.stats(id, function(err, stats) {
      ...
      if (err) {
        return error(500, 'unable to retreive stats');
      }
...
app.pipe = function(req, res, status, stream) {
  ...
  switch (res.getHeader('content-type')) {
    ...
    case 'text/javascript':
      stream.pipe(JSONStream.stringify('[', ',', ']')).pipe(JSONPstream(req.callback)).pipe(res);
      break;
    case 'text/csv':
    case 'text/plain':
    default:
