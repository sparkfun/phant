...
function PhantInput(config) {
  ...
  router.addRoute('/input/:streamId/clear.:ext', responder.route.bind(responder, 'clear'));
  router.addRoute('/input/:streamId/clear', responder.route.bind(responder, 'clear'));
  router.addRoute('/input/:streamId.:ext', responder.route.bind(responder, 'log'));
  router.addRoute('/input/:streamId', responder.route.bind(responder, 'log'));
...
/**** Defaults ****/
proto.moduleName = 'HTTP input';
proto.validator = false;
proto.keychain = false;
proto.requestLimit = 51200; // 50k
...
proto.log = function(req, res) {

  var data = {},
    streamId = req.streamId,
    publishKey = req.publishKey || req.publishToken;
  ...
  // check for publish key
  if (!publishKey) {
    return respond(403, 'forbidden: missing publish key');
  }
  ...
  // validate keys
  if (!this.keychain.validatePublish(streamId, publishKey)) {
    return respond(401, 'forbidden: authentication failed');
  }

  // get the meta id
  metaId = this.keychain.getMetaIdFromPublishKey(publishKey);

  // add timestamp
  data.timestamp = new Date().toISOString();

  this.throttler.available(streamId, function(ready, used, limit, reset) {
    ...
    input.validator.fields(metaId, data, function(err, valid) {

      if (!valid || res.headersSent) {
        return respond(400, err);
      }

      input.emit('data', metaId, data);
...
proto.clear = function(req, res) {

  var data = {},
    streamId = req.streamId,
    manageKey = req.manageKey || req.manageToken,
    respond = this.respond.bind(this, req, res),
    input = this,
    metaId = false;
  ...
  if (!manageKey) {
    return respond(403, 'forbidden: missing management key');
  }

  // validate keys
  if (!this.keychain.validateManage(streamId, manageKey)) {
    return respond(401, 'forbidden: authentication failed');
  }

  // get the metadata id
  metaId = this.keychain.getIdFromManageKey(manageKey);

  if(! metaId) {
    return respond(400, 'unable to clear stream');
  }

  input.emit('clear', metaId);

  respond(200, 'success');

};

proto.respond = function(req, res, status, message) {
...
proto.setContentType = function(req, res) {
...
proto.route = function(action, req, res, match) {
  ...
  req.streamId = match.params.streamId;
  req.publishKey = req.headers['phant-publish-key'];
  req.publishToken = req.headers['phant-publish-token'];
  req.manageKey = req.headers['phant-manage-key'];
  req.manageToken = req.headers['phant-manage-token'];
  ...
  delete req.query.publish_key;
  delete req.query.manage_key;
  delete req.query.delete_key;
