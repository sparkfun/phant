...
function PhantManager(port, config) {
  ...
  if(! (this instanceof PhantManager)) {
    return new PhantManager(port, config);
  }
...
client.create = function() {
  ...
  async.series({
    ...
    self.manager.metadata.create(self.new_stream, function (err, stream) {
      self.new_stream = {};
...
client.touch = function (id) {

  if (! this.manager.metadata) {
    return;
  }

  this.manager.metadata.touch(id, function (err) {
    if (err) {
      this.emit('error', err);
    }
  }.bind(this));
