...
app.create = function(stream, callback) {
  ...
  if (!stream.title.length) {
    return callback('title missing', false);
  }

  if (!stream.description.length) {
    return callback('description missing', false);
  }

  if (!stream.fields instanceof Array || !stream.fields.length) {
    return callback('fields missing', false);
  }

  if (stream.fields.length > 30) {
    return callback('too many fields sent. 30 fields max', false);
  }

  if (err = not_unique(stream.fields)) {
    return callback('duplicate field sent: ' + err, false);
  }

  if (err = invalid_names(stream.fields)) {
    return callback('invalid field name: ' + err + '. valid characters are _, a-z, A-Z, and 0-9', false);
  }

  if (!stream.tags instanceof Array) {
    return callback('invalid tags sent', false);
  }

  if (stream.tags.length > 15) {
    return callback('too many tags sent. 15 tags max', false);
  }

  if (err = not_unique(stream.tags)) {
    return callback('duplicate tag sent: ' + err, false);
  }

  if (err = invalid_names(stream.tags)) {
    return callback('invalid tag name: ' + err + '. valid characters are _, a-z, A-Z, and 0-9', false);
  }
