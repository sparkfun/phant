...
app.create = function(stream, callback) {
  ...
  if (err = invalid_names(stream.fields)) {
    return callback('invalid field name: ' + err + '. valid characters are _, a-z, A-Z, and 0-9');
  }
  ...
  if (err = invalid_names(stream.tags)) {
    return callback('invalid tag name: ' + err + '. valid characters are _, a-z, A-Z, and 0-9');
  }
